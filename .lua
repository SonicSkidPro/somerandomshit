...
jew = hookmetamethod(game, "__index", newcclosure(function(t, k)
    if t == Mouse and tostring(k) == "Hit" or tostring(k) == "Target" then
        if SilentAimSettings.enabled then
            ...
            local targetbone
            if target and 
            (not SilentAimSettings.fov.showfov or SilentAimSettings.fov.fovsize > pos) and 
            (not SilentAimSettings.wallcheck or wallcheck(target.Character.HumanoidRootPart, game.Players.LocalPlayer.Character.HumanoidRootPart)) and 
            ...
            (not SilentAimSettings.grabcheck or not grabbed(target)) then
                if SilentAimSettings.airshot then 
                    if target.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                        targetbone = target.Character[SilentAimSettings.airshotbone]
                    else
                ...
                local Prediction = targetbone.CFrame + targetbone.Velocity * SilentAimSettings.prediction
